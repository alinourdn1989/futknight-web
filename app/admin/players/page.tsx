      ...
      <main className="flex-1 md:ml-56 px-4 md:px-10 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 text-sm mt-0.5">
              {players.length} total 路 {players.filter(p => p.user_id).length} registered 路 {players.filter(p => p.api_player_id).length} linked to real players
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center mt-32">
            <p className="text-4xl mb-4">馃懃</p>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gray-500 text-xs">
                          {result.statistics[0]?.team.name} 路 {result.statistics[0]?.games.position} 路 {result.player.nationality}
                        </p>
                      </div>
            ...
            {selectedApiPlayer && (
              <div className="flex items-center gap-3 bg-[#0A0A0A] border border-cyan-400 rounded-xl p-3 mb-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-500 text-xs">{selectedApiPlayer.statistics[0]?.team.name} 路 {selectedApiPlayer.player.nationality}</p>
                </div>
                <button onClick={() => { setSelectedApiPlayer(null); setSearchQuery(""); }} className="text-gray-600 hover:text-red-400 text-lg">鉁?/button>
              </div>
            )}
    ...
    <div className="bg-[#111] border border-[#1A1A1A] rounded-2xl p-5 hover:border-[#333] transition group">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {p.nationality && <p className="text-gray-600 text-xs">{p.nationality} 路 {p.position}</p>}
        </div>
        {p.rating && (
      ...
      <div className="border-t border-[#1A1A1A] pt-3 flex items-center justify-between">
        <div>
          ...
          <p className={`text-xs font-bold mt-0.5 ${p.user_id ? "text-cyan-400" : "text-orange-500"}`}>
            {p.user_id ? "鉁?Registered" : "鈴?Pending"}
          </p>
        </div>
        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition">
          ...
          <button onClick={onDelete} className="text-gray-400 hover:text-red-400 text-xs px-3 py-1.5 bg-[#1A1A1A] rounded-lg border border-[#333]">鉁?/button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-[#111] rounded-xl p-4 border border-[#222] flex items-center gap-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        {p.club && <p className="text-gray-500 text-xs">{p.club} 路 {p.position}</p>}
      ...
      <div className="flex gap-2 shrink-0">
        ...
        <button onClick={onDelete} className="text-gray-400 hover:text-red-400 text-sm px-3 py-1.5 bg-[#1A1A1A] rounded-lg border border-[#333]">鉁?/button>
      </div>
    </div>
  );
}