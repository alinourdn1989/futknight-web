...
export default function PublicTournament() {
  ...
  const [tab, setTab] = useState<"fixtures" | "standings" | "scorers">("fixtures");
  ...
  const fetchAll = useCallback(async () => {
    ...
    const { data: matchesData } = await supabase.from("matches").select("*")
      .eq("tournament_id", id).order("round", { ascending: true });
    ...
    if (goals) {
      ...
      const sorted = Object.entries(scorerMap)
        .map(([name, data]) => ({
          player_name: name,
          teamName: teamsData?.find(t => t.id === data.team_id)?.name || "",
          goals: data.goals,
        }))
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A]">
      ...
      <nav className="border-b border-[#111] px-4 md:px-10 py-4 flex justify-between items-center sticky top-0 bg-[#0A0A0A]/95 backdrop-blur-sm z-40">
        <span className="text-cyan-400 font-extrabold text-lg">⚔️ FutKnight</span>
        <button onClick={copyLink}
          className={"flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition border " + (
            copied
              ? "bg-cyan-400 text-black border-cyan-400"
              : "bg-[#111] text-gray-400 border-[#333] hover:border-cyan-400 hover:text-cyan-400"
          )}>
          {copied ? "✓ Copied!" : "🔗 Share"}
        </button>
      </nav>
      ...
      <main className="px-4 md:px-10 py-8 max-w-5xl mx-auto">
        ...
        <div className="flex border-b border-[#222] mb-6">
          <button onClick={() => setTab("fixtures")}
            className={"px-5 py-2.5 font-bold text-sm " + (tab === "fixtures" ? "text-cyan-400 border-b-2 border-cyan-400" : "text-gray-600 hover:text-gray-400")}>
            Fixtures
          </button>
          {tournament?.format === "round_robin" && (
            <button onClick={() => setTab("standings")}
              className={"px-5 py-2.5 font-bold text-sm " + (tab === "standings" ? "text-cyan-400 border-b-2 border-cyan-400" : "text-gray-600 hover:text-gray-400")}>
              Standings
            </button>
          )}
          <button onClick={() => setTab("scorers")}
            className={"px-5 py-2.5 font-bold text-sm " + (tab === "scorers" ? "text-cyan-400 border-b-2 border-cyan-400" : "text-gray-600 hover:text-gray-400")}>
            Top Scorers
          </button>
        </div>
            ...
            <div>
              ...
              {standings.map((s, i) => (
                <div key={s.teamId} className={"flex items-center py-3.5 px-4 rounded-lg " + (i % 2 === 0 ? "bg-[#111]" : "")}>
                  ...
                  <span className="flex-1 text-white font-bold text-sm flex items-center gap-2">
                    {s.teamName}
